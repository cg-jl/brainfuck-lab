...
uint8_t find_div_noprime(uint8_t val, uint8_t *prime_diff) {
  uint8_t div = highest_div_pair(val);
  *prime_diff = 0;
  while (div == 1) {
    (*prime_diff)++;
    val--;
    div = highest_div_pair(val);
  }
  return div;
}
...
void byte(uint8_t next, uint8_t last) {
  ...
  uint8_t max, min, prime_diff;
  {
    uint8_t div1 = find_div_noprime(diff, &prime_diff);
  ...
  for (uint8_t i = 0; i < prime_diff; i++) putchar(direction);
